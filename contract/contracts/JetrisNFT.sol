...
contract JetrisNFT is Ownable, ERC721Enumerable {
    ...
    constructor(
        ...
        string memory _JetrisBaseURI, 
        address payable _treasuryWallet)  ERC721("JetrisNFT", "NFT") {

        marketContractAddress = _marketAddress;
        mintPrice = 1;
        maxTokenNum = 20; // constructor의 가스비 절약을 위해
        _setToken(_tokenContractAddress);
        _setBaseURI(_JetrisBaseURI);
        _generateMintArray();
    ...
    // 민팅 페이어블 
    function mintJetrisNFT() public payable returns (uint256)  {
    ...
    function _setBaseURI(string memory _JetrisBaseURI) private onlyOwner {
        baseURIextended = _JetrisBaseURI;
    }
    ...
    // NFT mint -> JetToken 으로.
    function _setToken(address jtokenAddress) private onlyOwner returns (bool) {
        require(jtokenAddress != address(0x0));
        token = JetToken(jtokenAddress);
        return true;
    }
