...
contract JetToken is ERC20 {
    ...
    address public JetrisNFT;
    address public JetrisItems;
    address public JetrisMarket;
    address public JetrisUser;
    ...
    modifier jetrisNFTOnly(){
        require(msg.sender == JetrisNFT || msg.sender == JetrisItems || msg.sender == JetrisMarket, "JTRISNFT_ONLY");
        _;
    }
    // router 컨트랙트에서만 실행
    modifier routerOnly() {
        require(msg.sender == Router || msg.sender == JetrisNFT || msg.sender == JetrisItems || msg.sender == JetrisMarket || msg.sender == JetrisUser, "ROUTER_OR_Jetris_ONLY");
        _;
    }
    // 펀딩 종료 체크 
    ...
    function setJetrisNFTAddress(address _jetrisNft) external ownerOnly {
        require(address(JetrisNFT) == address(0), "WRITE_ONCE!");
        JetrisNFT = _jetrisNft;
    }
    function setJetrisItemsAddress(address _jetrisItems) external ownerOnly {
        require(address(JetrisItems) == address(0), "WRITE_ONCE!");
        JetrisItems = _jetrisItems;
    }
    function setJetrisMarketAddress(address _jetrisMarket) external ownerOnly {
        require(address(JetrisMarket) == address(0), "WRITE_ONCE!");
        JetrisMarket = _jetrisMarket;
    }
    function setJetrisUserAddress(address _jetrisUser) external ownerOnly {
        require(address(JetrisUser) == address(0), "WRITE_ONCE!");
        JetrisUser = _jetrisUser;
    }
    ...
    function marketBurn(address account, uint256 amount) external jetrisNFTOnly {
    ...
    function sendLiquidityToLPContract(LiquidityPool liquidityPool) external ownerOnly areFundsMoved
    {
        ...
        // 이더 기부금 * 100 jt 에어드랍
        uint256 jtCoinAmountToTransfer = totalContributed * 100;

        // lp풀 JT전송 
        super._transfer(
            address(this),
            address(liquidityPool),
            jtCoinAmountToTransfer
        );
    ...
    //                                                                          펀딩 reserve_setting
     ...
     //                                      펀딩 이후 남은 토큰 재무 관련 지갑 전송 오너지갑으로(우선은...)
    
    function sendRemainingFundsToTreasury() internal {
        MovedStakingReward();

        uint256 remainingJT = balanceOf(address(this));
        super._transfer(address(this), address(treasuryWallet), remainingJT);
        emit FundsMoved();
    }
    // staking reward 300,000개 발행 
    function MovedStakingReward() internal {
        uint256 remainingJT = 300000 * 10**decimals();
        super._transfer(address(this), address(Staking), remainingJT);
    }
